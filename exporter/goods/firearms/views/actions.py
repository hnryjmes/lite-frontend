...
from exporter.core.helpers import (
    ...
    get_organisation_rfd_certificate,
...
class FirearmActCertificateAction:
    def __init__(self, document_type, view, cleaned_data):
        ...
        self.organisation = view.organisation
        self.good = view.good
    ...
    def has_existing_firearm_act_certificate(self):
        return has_organisation_firearm_act_document(self.organisation, self.document_type)
    ...
    def get_firearm_act_certificate_payload(self):
        ...
        payload = {
            "document": get_document_data(certificate),
    ...
    def post_firearm_act_certificate(self):
        ...
        return post_document_on_organisation(
    ...
    def update_firearm_act_certificate(self):
        document = get_organisation_firearm_act_document(self.organisation, self.document_type)
    ...
    def delete_existing_organisation_firearm_act_certificate(self):
        certificate_data = get_organisation_firearm_act_document(self.organisation, self.document_type)
...
def delete_existing_organisation_rfd_certificate(request, organisation):
    organisation_rfd_certificate_data = get_organisation_rfd_certificate(organisation)
    status_code = delete_document_on_organisation(
        request,
...
def delete_existing_application_rfd_certificate(request, organisation):
    organisation_rfd_certificate_data = get_organisation_rfd_certificate(organisation)
...
class IsRfdAction:
    def __init__(self, step_name, wizard):
        ...
        self.organisation = wizard.organisation
    ...
    def has_existing_certificate(self):
        return has_valid_organisation_rfd_certificate(self.organisation)
    ...
    def delete_existing_organisation_rfd_certificate(self):
        return delete_existing_organisation_rfd_certificate(self.request, self.organisation)
    ...
    def delete_existing_application_rfd_certificate(self):
        return delete_existing_application_rfd_certificate(self.request, self.organisation)
...
class RfdCertificateAction:
    ...
    def get_organisation_rfd_certificate_payload(self):
        rfd_certificate_cleaned_data = self.wizard.get_cleaned_data_for_step(self.step_name)
        cert_file = rfd_certificate_cleaned_data["file"]
        expiry_date = rfd_certificate_cleaned_data["expiry_date"]
        reference_code = rfd_certificate_cleaned_data["reference_code"]
        ...
        rfd_certificate_payload = {
            "document": get_document_data(cert_file),
            "expiry_date": expiry_date.isoformat(),
            "reference_code": reference_code,
            "document_type": DocumentType.RFD_CERTIFICATE,
        }
    ...
    def post_rfd_certificate(self):
        rfd_certificate_payload = self.get_organisation_rfd_certificate_payload()
        return post_document_on_organisation(
            request=self.request,
            organisation_id=self.organisation["id"],
    ...
    def update_rfd_certificate(self):
        rfd_document = get_organisation_rfd_certificate(self.organisation)
